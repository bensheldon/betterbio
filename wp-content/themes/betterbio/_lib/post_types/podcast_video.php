 ...
 */
add_action( 'init', 'create_podcast_video_type' );
add_action( 'init', 'create_podcast_video_taxonomies', 0 );

function create_podcast_video_type() {
  register_post_type( 'podcast_video',
    array(
      'labels' => array(
        'name' => __( "Video Podcasts" ),
        'singular_name' => __( "Video Podcast" ),
      ),
      'public' => true,
      'show_ui' => true,
      'capability_type' => 'post',
      'taxonomies' => array("podcast_video_categories"),
      'supports' => array('podcast_video_categories','title','editor','thumbnail','comments','revisions')
    )
  );
  flush_rewrite_rules();
}
...
function create_podcast_taxonomies() 
{
  $labels = array(
    ...
    'singular_name' => _x( 'Podcast', 'Podcast singular name' ),
    ...
    'new_item_name' => __( 'New Podcast' ),
    'menu_name' => __( 'Podcast' ),
  ); 	

  register_taxonomy('podcast_tags',array('podcast_tag'), array(
    'hierarchical' => true,
    'labels' => $labels,
    'show_ui' => true,
    'query_var' => true,
...
if (is_admin()) add_action('admin_menu', 'add_podcast_boxen');
function add_podcast_boxen() {
