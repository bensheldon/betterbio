...
function cp_addThemePage() {
  ...
  $operator = 'and'; // 'and' or 'or'
      ...
      update_option( 'ar_video', $_REQUEST[ 'ar_video' ] );
            ...
            $cp_categories = get_post_types($args,$output,$operator);
            $cp_numCats = 0;

            for( $cp_x = 1; $cp_x <= sizeof($cp_categories); $cp_x ++ ) {
                ...
                update_option( 'cp_descText_' . $cp_x, $_REQUEST[ 'cp_descText_' . $cp_x ] );
               }
			...
			header("Location: themes.php?page=themepage.php&saved=true");
			die;
  		...
  		// reset settings
			...
			// goto theme edit page
			header("Location: themes.php?page=themepage.php&reset=true");
			die;
...
function cp_themePage() {
  ...
  $operator = 'and'; // 'and' or 'or'
	...
	<?php 
		...
		$cp_categories = get_post_types($args,$output,$operator); 
		foreach ( $cp_categories as $key => $name ) { 
...
?>
    ...
    $cp_numCats = 0;
		...
		foreach ( $cp_categories as $pt ) {
			$cp_Colorcat[] = array( $pt->name, ucfirst($pt->name) );
            $cp_numCats ++;
		}
