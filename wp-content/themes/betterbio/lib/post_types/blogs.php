 ...
 */
add_action( 'init', 'create_blogs_type' );
add_action( 'init', 'create_blogs_taxonomies', 0 );

function create_blogs_type() {
...
{
  $labels = array(
    'name' => _x( 'Categories', 'Blog Post Categories' ),
    'singular_name' => _x( 'Category', 'Blog' ),
  ...
  register_taxonomy('blogs_categories',array('blogs_category'), array(
    'hierarchical' => true,
    'labels' => $labels,
    'show_ui' => true,
    'query_var' => true,
...
if (is_admin()) add_action('admin_menu', 'add_blogs_boxen');
function add_blogs_boxen() {
